...
class LunarLanderEnv(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        ''' MR: We dont need this for sparse reward scenarios
        reward = 0
        shaping = (
        ...
        reward -= s_power * 0.03
        '''
        reward = 0.0  # no reward before termination
        terminated = False
        ...
        if not self.lander.awake:
            terminated = True
            reward = 100.0 * (0.999 ** self.steps_taken)  # Scale the reward by the time taken

        truncateds = False
        # Stop run if lander takes too long
        if self.steps_taken > 1000:
            truncateds = True

        if self.render_mode == "human":
            self.render()
        return np.array(state, dtype=np.float32), reward, terminated, truncateds, {}
...
if __name__ == "__main__":
    demo_heuristic_lander(LunarLanderEnv(render_mode="human"), render=True)
