...
def evolvePop(self):
    ...
    """
    newPop = []
    #  The loop iterates over each species in the self.species list, recombines the individuals within
    #  each species to produce children, and appends these children to a new population list
...
def recombine(self, species, innov, gen):
  ...
  Procedure:
    ) Sort all individuals by rank (Pareto dominance ranking of individuals)
  ...
  """
    ...
    for i in range(nOffspring):
        # Decide whether to perform mutation only or crossover. Based on the crossover probability perform mutation or crossover only
...
def crossover(self, parentA, parentB):
    """Combine genes of two individuals to produce new individual following the idea of NEAT.

    Procedure:
    ) Inherit all nodes and connections from most fit parent parentA
    ) Identify matching connection genes in parentA and parentB
      (Matching genes are those with the same innovation number)
    ) For each matching gene, the weight from parentB is taken with some probability
